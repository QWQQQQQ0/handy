// 来源: lib/screens/web_screen.dart + lib/providers/web_provider.dart
    ...
    <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="px-3 mt-3">
          <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 p-3">
            ...
            {mode === 'coordinate' ? (
              <>
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    type="number"
                    placeholder="X"
                    className="flex-1 px-2 py-1.5 text-[13px] rounded border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 outline-none focus:border-blue-500"
                  />
                  <input
                    ref={yRef}
                    type="number"
                  ...
                  <button
                    ...
                    disabled={!connected}
                    className="px-3 py-1.5 rounded bg-blue-600 text-white text-[13px] font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1"
                  >
              ...
              <>
                <input
                  ref={selectorRef}
                  type="text"
                  placeholder="CSS Selector (#search, .btn, input[name='q'])"
                  className="w-full px-2 py-1.5 text-[13px] rounded border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 outline-none focus:border-blue-500 mb-2"
                />
                <div className="flex flex-wrap gap-1.5 mb-2">
                  <button onClick={handleSelectorClick} disabled={!connected} className="px-2.5 py-1 rounded bg-blue-600 text-white text-[12px] font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1">
                    <MousePointer size={12} /> Click
                  </button>
                  <button onClick={handleFill} disabled={!connected} className="px-2.5 py-1 rounded bg-blue-600 text-white text-[12px] font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1">
                    <Keyboard size={12} /> Fill
                  </button>
                  <button onClick={handleExtract} disabled={!connected} className="px-2.5 py-1 rounded bg-blue-600 text-white text-[12px] font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1">
                    <Search size={12} /> Extract
                  </button>
                  <button onClick={handleScrollIntoView} disabled={!connected} className="px-2.5 py-1 rounded bg-blue-600 text-white text-[12px] font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1">
                    <ChevronDown size={12} /> Scroll To
                  </button>
                </div>
                <input
                  ref={typeRef}
            ...
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => handleScroll(300)} className="px-2.5 py-1 rounded border border-zinc-200 dark:border-zinc-700 text-[12px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-1">
                <ChevronDown size={12} /> Scroll Down
              </button>
              <button onClick={() => handleScroll(-300)} className="px-2.5 py-1 rounded border border-zinc-200 dark:border-zinc-700 text-[12px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-1">
                <ChevronUp size={12} /> Scroll Up
              </button>
              <button onClick={() => handlePressKey('Enter')} className="px-2.5 py-1 rounded border border-zinc-200 dark:border-zinc-700 text-[12px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-1">
                <CornerDownLeft size={12} /> Enter
              </button>
              <button onClick={() => handlePressKey('Escape')} className="px-2.5 py-1 rounded border border-zinc-200 dark:border-zinc-700 text-[12px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-1">
                Esc
              </button>
              <button onClick={() => handlePressKey('Tab')} className="px-2.5 py-1 rounded border border-zinc-200 dark:border-zinc-700 text-[12px] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-1">
                Tab
              </button>
            </div>
          </div>
      ...
      {showDomDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowDomDialog(false)}>
          <div
            className="bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-800 w-[600px] max-h-[70vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
              <h3 className="text-[14px] font-semibold text-zinc-900 dark:text-zinc-100">
                DOM Tree ({domNodes.length} elements)
              </h3>
              <button
                onClick={() => setShowDomDialog(false)}
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {domNodes.map((n, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => sel && handleSelectNode(sel)}
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center gap-3"
                  >
                    <span className="px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-950 text-[10px] font-bold text-blue-700 dark:text-blue-300 shrink-0">
                      {tag}
                    </span>
                    ...
                    {sel && (
                      <span className="text-[10px] text-zinc-300 dark:text-zinc-600 font-mono truncate max-w-[120px] shrink-0">
                        {sel}
                      </span>
                    )}
                  </button>
