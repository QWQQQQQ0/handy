...
from engine.sandbox import _handle_exec_python, _handle_web_code_exec, _handle_doc_code_exec
...
def _handle_web_heartbeat(_params: dict) -> dict[str, Any]:
    """Drive Playwright event loop (called by frontend every ~500ms)."""
    _get_browser().heartbeat()
    return {"ok": True}
...
TOOL_MAP: dict[str, Callable[[dict], dict[str, Any]]] = {
    ...
    "web_heartbeat":       _handle_web_heartbeat,
